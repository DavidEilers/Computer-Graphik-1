...
print_info_log(GLuint obj, bool program)
{
	char log[16384];
	if (program) {
		glGetProgramInfoLog(obj, 16383, NULL, log);
	} else {
		glGetShaderInfoLog(obj, 16383, NULL, log);
	}
	log[16383]=0;
	fprintf(stderr,"%s\n",log);
}
...
bool CG1Helper::initApplication(Application* application, int windowScale)
{
	...
	int wh, hh;
	const int& width = application->context.viewport.width;
	const int& height = application->context.viewport.height;
	...
	if (glcaps.supports(CGLCaps::PBO)) {
		...
		glBufferData(GL_PIXEL_UNPACK_BUFFER, 4*width*height, NULL, GL_STREAM_DRAW);
	...
	glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)sizeof(GLfloat)*4*(width+1 + height+1), NULL, GL_STATIC_DRAW);
...
{
	timespec timeBetweenFrames = {0,15000000};
...
void CG1Helper::m_displayFunc()
{
	...
	if(smFrameBuffer->colorBuffer.getDataPointer()) {
		...
		if (glcaps.supports(CGLCaps::PBO)) {
			...
			glTexSubImage2D(GL_TEXTURE_2D,0,0,0,smFrameBuffer->getWidth(),smFrameBuffer->getHeight(),GL_RGBA,GL_UNSIGNED_BYTE,NULL);
		} else {
...
Application* CG1Helper::smApplication = NULL;
