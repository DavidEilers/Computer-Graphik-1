...
public:
	...
	static CGMatrix4x4 getFrustum(float left, float right, float bottom, float top, float nearVal, float farVal);
	///  returns an orthogonal projection matrix for a specified frustum
	static CGMatrix4x4 getOrtho(float left, float right, float bottom, float top, float nearVal, float farVal);
	/// returns a symmetrical perspecitive matrix for the specified parameters
	static CGMatrix4x4 getPerspective(float fov_y, float aspect, float zNear, float zFar);
	/// retruns a view matrix for the specified camera position and orientation
	static CGMatrix4x4 getLookAt(const CGVec4& eye, const CGVec4& center, const CGVec4 up);
	...
	/// (read and write)
	float& at(const unsigned int c, const unsigned int r)
	{
		assert(c<4&&r<4);
		return m_matrix[c][r];
	}
