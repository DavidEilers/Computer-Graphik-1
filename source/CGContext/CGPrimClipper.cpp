...
// Init static members
//----------------------------------------------------------------------------
CGVaryings CGPrimClipper::sm_voutBuf[CG_MAX_VERTICES_IN_PIPELINE];
unsigned int CGPrimClipper::ClipBits = 0xffff;
//----------------------------------------------------------------------------
// Implementation of member functions
//----------------------------------------------------------------------------
const CGVaryings *CGPrimClipper::outBuf() {return &sm_voutBuf[0];}

unsigned int CGPrimClipper::clipPoint(CGVaryings *vinBuf, int vinCnt)
{
...
//----------------------------------------------------------------------------
unsigned int CGPrimClipper::clipLine(CGVaryings *vinBuf, int vinCnt)
{
	// use Liang-Barsky line-clipping for now.
	return clipLine_LB(vinBuf, vinCnt);
}
//----------------------------------------------------------------------------
unsigned int CGPrimClipper::clipPoly(CGVaryings *vinBuf, int vinCnt)
{
	// use Sutherland-Hodgman polygon-clipping for now.
	return clipPoly_SH(vinBuf, vinCnt);
}
//----------------------------------------------------------------------------
unsigned int CGPrimClipper::clipLine_LB(CGVaryings *vinBuf, int vinCnt)
{
	...
	CGVaryings *vs = NULL, *ve = NULL;       //the line segment to be checked is always vs->ve
	const CGVec4 *ps = NULL, *pe = NULL;       //positions of vertices, for boundary test and intersection
...
//----------------------------------------------------------------------------
unsigned int CGPrimClipper::clipPoly_SH(CGVaryings *vinBuf, int vinCnt)
{
	...
	// Init out buffer
	CGVaryings *voutBuf = &sm_voutBuf[0], *tmp = NULL;

	// Do clipping
	for (unsigned int i=0; i<ClipBound::Cnt; ++i) {
		if (! (vinCnt = clipPoly_SH_single(clipOrder[i], vinBuf, vinCnt, voutBuf)) ) {
...
//----------------------------------------------------------------------------
int CGPrimClipper::clipPoly_SH_single(unsigned int clipBound, const CGVaryings *vinBuf, int nvin, CGVaryings *voutBuf)
{
	int voutCnt = 0;
	CGVaryings *vout = voutBuf;        //current output vertex
	float t=0.0f;                            //ratio for interpolation
	const CGVaryings *vs = NULL, *ve = NULL; //the line segment to be checked is always vs->ve
	const CGVec4 *ps = NULL, *pe = NULL;       //positions of vertices, for boundary test and intersection
